// ==UserScript==
// @name         Windows Central - suppress anti-adblock prompt and reading-position reset
// @description  Suppress the Windows Central "Please allow ads" prompt and fight forced reload/scroll jumps back to the article start.
...
(() => {
  ...
  const pageKey = `wc-pos:v3:${location.pathname}${location.search}`;
  const now = () => Date.now();
  ...
  let savedY = readSaved();
  let lastGoodY = Math.max(savedY, getY());
  ...
  window.alert = value => shouldBlockPrompt(value) ? undefined : originalAlert(value);
  window.confirm = value => shouldBlockPrompt(value) ? false : originalConfirm(value);
  ...
  const shouldBlockProgrammaticJump = targetY => {
    ...
    // Block jumps to the top OR to the article start if they move us sharply upward.
  ...
  const canRestore = y => {
    if (!Number.isFinite(y) || y < 250) return false;
    // Wait until the article is tall enough after reload/lazy layout.
    return maxScrollableY() >= y - 120;
  };
  ...
  history.go = delta => {
    if (now() <= guardUntil && Number(delta) === 0) return undefined;
  ...
  // Save continuously; iOS Safari can skip final lifecycle events during forced reloads.
  setInterval(remember, 250);

  // Restore aggressively after reload and after lazy layout shifts.
  const restoreSchedule = [0, 25, 50, 100, 200, 350, 500, 750, 1000, 1500, 2200, 3000, 4500, 6500, 9000, 12000, 16000, 22000, 30000];
  restoreSchedule.forEach(delay => setTimeout(restore, delay));

  // During the guard window, keep pulling the reader back if the site keeps snapping upward.
